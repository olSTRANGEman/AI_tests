...
def test_get_pet_by_id_valid():
    pet_id = unique_id()
    # create first
    requests.post(f"{BASE_URL}/pet", json={'id': pet_id, 'name': 'x', 'photoUrls': ['u'], 'status': 'available'})
...
def test_update_pet_put_valid():
    pet_id = unique_id()
    requests.post(f"{BASE_URL}/pet", json={'id': pet_id, 'name': 'orig', 'photoUrls': ['u'], 'status': 'available'})
    payload = {'id': pet_id, 'name': 'updated', 'photoUrls': ['u'], 'status': 'sold'}
    resp = requests.put(f"{BASE_URL}/pet", json=payload)
    assert resp.status_code == 200
    assert resp.json()['name'] == 'updated'

def test_update_pet_put_invalid_id():
    resp = requests.put(f"{BASE_URL}/pet", data={'id': 'nonint', 'name': 'n', 'photoUrls': ['u'], 'status': 'available'})
    assert resp.status_code == 415

def test_delete_pet_valid():
    pet_id = unique_id()
    requests.post(f"{BASE_URL}/pet", json={'id': pet_id, 'name': 't', 'photoUrls': ['u'], 'status': 'available'})
...
def test_place_order_valid():
    order_id = unique_id()
    payload = {'id': order_id, 'petId': order_id, 'quantity': 1, 'status': 'placed'}
    resp = requests.post(f"{BASE_URL}/store/order", json=payload)
    assert resp.status_code == 200
    assert resp.json()['id'] == order_id

def test_place_order_missing_field():
    resp = requests.post(f"{BASE_URL}/store/order", data={'petId': 1, 'quantity': 1})
    assert resp.status_code == 415

def test_get_order_by_id_valid():
    order_id = unique_id()
    requests.post(f"{BASE_URL}/store/order", json={'id': order_id, 'petId': order_id, 'quantity': 1})
...
def test_create_user_missing_username():
    resp = requests.post(f"{BASE_URL}/user", data={'id': unique_id(), 'firstName': 'F'})
    assert resp.status_code == 415
...
def test_login_user_valid():
    username = f"u{unique_id()}"
    password = 'pass'
    requests.post(f"{BASE_URL}/user", json={'id': unique_id(), 'username': username, 'firstName': 'F', 'email': 'e', 'password': password, 'phone': 'p'})
...
def test_get_user_by_name_valid():
    username = f"u{unique_id()}"
    requests.post(f"{BASE_URL}/user", json={'id': unique_id(), 'username': username, 'firstName': 'F', 'email': 'e', 'password': 'p', 'phone': '1'})
...
def test_update_user_valid():
    username = f"u{unique_id()}"
    requests.post(f"{BASE_URL}/user", json={'id': unique_id(), 'username': username, 'firstName': 'F', 'email': 'e', 'password': 'p', 'phone': '1'})
    resp = requests.put(f"{BASE_URL}/user/{username}", data={'firstName': 'FF', 'lastName': 'LL'})
    assert resp.status_code == 200
...
def test_crud_pet_flow():
    pet_id = unique_id()
    # Create
    resp = requests.post(f"{BASE_URL}/pet", json={'id': pet_id, 'name': 'flow', 'photoUrls': ['u'], 'status': 'available'})
    assert resp.status_code == 200
    # Read
    resp = requests.get(f"{BASE_URL}/pet/{pet_id}")
    assert resp.status_code == 200
    # Update
    resp = requests.put(f"{BASE_URL}/pet", json={'id': pet_id, 'name': 'flow2', 'photoUrls': ['u'], 'status': 'sold'})
    assert resp.status_code == 200
    # Delete
    resp = requests.delete(f"{BASE_URL}/pet/{pet_id}")
    assert resp.status_code == 200
...
def test_auth_and_access_flow():
    username = f"u{unique_id()}"
    password = 'p'
    requests.post(f"{BASE_URL}/user", json={'id': unique_id(), 'username': username, 'firstName': 'F', 'email': 'e', 'password': password, 'phone': '1'})
    resp = requests.get(f"{BASE_URL}/user/login", params={'username': username, 'password': password})
    assert resp.status_code == 200
...
def test_large_payload():
    ...
    payload = {'id': pet_id, 'name': large_name, 'photoUrls': ['u'], 'status': 'available'}
    resp = requests.post(f"{BASE_URL}/pet", json=payload)
