...
def test_get_order_by_id_valid():
    pet = create_test_pet()
    pet_id = pet["id"]
    order = create_test_order(pet_id)
    order_id = order["id"]
...
def test_update_order_valid():
    ...
    response = requests.put(f"{BASE_URL}/store/order", data=updated_order)
    assert response.status_code == 200
...
def test_update_user_valid():
    ...
    response = requests.put(f"{BASE_URL}/user", data=updated_user)
    assert response.status_code == 200
...
def test_create_user_with_array():
    ...
    response = requests.post(f"{BASE_URL}/user/createWithArray", data=users_data[0])
    assert response.status_code == 200
    
    response = requests.post(f"{BASE_URL}/user/createWithList", data=users_data)
    assert response.status_code == 200
