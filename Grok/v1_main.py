...
# Helper function to generate form data string from dict (since JSON is prohibited)
...
def test_post_pet_valid():
    ...
    response = requests.post(f"{BASE_URL}/pet", data=dict_to_form_data(pet_data), headers=headers, timeout=TIMEOUT)
...
def test_post_pet_invalid_data():
    headers = {"api_key": API_KEY, "Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/pet", data="invalid=data", headers=headers, timeout=TIMEOUT)
...
def test_get_pet_by_id_valid():
    pet_data = generate_pet_data()
    headers = {"api_key": API_KEY, "Content-Type": "application/x-www-form-urlencoded"}
    post_response = requests.post(f"{BASE_URL}/pet", data=dict_to_form_data(pet_data), headers=headers, timeout=TIMEOUT)
...
def test_put_pet_valid():
    ...
    response = requests.put(f"{BASE_URL}/pet", data=dict_to_form_data(updated_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
    assert response.json()["name"] == updated_data["name"]

def test_put_pet_invalid_data():
    headers = {"api_key": API_KEY, "Content-Type": "application/x-www-form-urlencoded"}
    response = requests.put(f"{BASE_URL}/pet", data="invalid=data", headers=headers, timeout=TIMEOUT)
    assert response.status_code == 400

def test_put_pet_missing_api_key():
    pet_data = generate_pet_data()
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.put(f"{BASE_URL}/pet", data=dict_to_form_data(pet_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 401
...
def test_post_pet_find_by_status_valid():
...
def test_post_pet_upload_image_valid():
    ...
    response = requests.post(f"{BASE_URL}/pet/{pet_id}/uploadImage", files=files, headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
...
def test_post_store_order_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/store/order", data=dict_to_form_data(order_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
    assert response.json()["id"] == order_data["id"]

def test_post_store_order_invalid_data():
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/store/order", data="invalid=data", headers=headers, timeout=TIMEOUT)
    assert response.status_code == 400
...
def test_get_store_order_by_id_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    post_response = requests.post(f"{BASE_URL}/store/order", data=dict_to_form_data(order_data), headers=headers, timeout=TIMEOUT)
...
def test_post_user_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200

def test_post_user_invalid_data():
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/user", data="invalid=data", headers=headers, timeout=TIMEOUT)
    assert response.status_code == 400
...
def test_get_user_by_name_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
...
def test_put_user_valid():
    ...
    response = requests.put(f"{BASE_URL}/user/{user_data['username']}", data=dict_to_form_data(updated_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
...
def test_put_user_not_found():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.put(f"{BASE_URL}/user/nonexistent", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 404
...
def test_post_create_with_array_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/user/createWithArray", data=dict_to_form_data({"users": user_data[0]}), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
...
def test_post_create_with_list_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = requests.post(f"{BASE_URL}/user/createWithList", data=dict_to_form_data({"users": user_data[0]}), headers=headers, timeout=TIMEOUT)
    assert response.status_code == 200
...
def test_get_user_login_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    response = requests.get(f"{BASE_URL}/user/login?username={user_data['username']}&password=testpass", timeout=TIMEOUT)
    assert response.status_code == 200
...
def test_get_user_logout_valid():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    requests.get(f"{BASE_URL}/user/login?username={user_data['username']}&password=testpass", timeout=TIMEOUT)
...
def test_crud_pet_flow():
    ...
    updated_data = pet_data.copy()
    updated_data["name"] = "updated_" + pet_data["name"]
    put_response = requests.put(f"{BASE_URL}/pet", data=dict_to_form_data(updated_data), headers=headers, timeout=TIMEOUT)
...
def test_crud_store_order_flow():
    ...
    assert get_response.status_code == 200
    # Update (not directly supported, so skip)
    # Delete
...
def test_crud_user_flow():
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    # Create
    post_response = requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    assert post_response.status_code == 200
    # Read
    ...
    updated_data = user_data.copy()
    updated_data["firstName"] = "Updated"
    put_response = requests.put(f"{BASE_URL}/user/{user_data['username']}", data=dict_to_form_data(updated_data), headers=headers, timeout=TIMEOUT)
    assert put_response.status_code == 200
    # Delete
...
def test_auth_and_access_flow():
    ...
    requests.post(f"{BASE_URL}/user", data=dict_to_form_data(user_data), headers=headers, timeout=TIMEOUT)
    # Login
    login_response = requests.get(f"{BASE_URL}/user/login?username={user_data['username']}&password=testpass", timeout=TIMEOUT)
...
def test_large_payload_pet_post():
    headers = {"api_key": API_KEY, "Content-Type": "application/x-www-form-urlencoded"}
    large_data = {"name": "a" * 10000, "id": random.randint(1000, 9999)}
    response = requests.post(f"{BASE_URL}/pet", data=dict_to_form_data(large_data), headers=headers, timeout=TIMEOUT)
...
def test_large_payload_user_post():
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    large_data = {"username": "a" * 10000, "id": random.randint(1000, 9999)}
    response = requests.post(f"{BASE_URL}/user", data=dict_to_form_data(large_data), headers=headers, timeout=TIMEOUT)
    assert response.status_code in [200, 400, 413]  # Allow for payload size rejection
